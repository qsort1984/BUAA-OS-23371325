...
static void map_segment(Pde *pgdir, u_int asid, u_long pa, u_long va, u_int size, u_int perm) {
	...
	for (int i = 0; i < size; i += PAGE_SIZE) {
		...
		panic_on(page_insert(pgdir, asid, pp, va + i, perm));
	}
}
...
void env_init(void) {
	...
	LIST_INTT(&env_free_list);
	TAILQ_INIT(&env_shed_list);
	...
	for (i = NENV - 1; i >= 0; i--) {
			...
			TAILQ_INSERT_HEAD(&env_free_list, &envs[i], env_link);
	}
...
static int load_icode_mapper(void *data, u_long va, size_t offset, u_int perm, const void *src,
	...
	if (src != NULL) {
		...
		memcpy((char *)page2kva(p) + offset, src, len);
	}
